...
export async function main(ns) {
	...
	var exclude = ['home', 'hacknet-node-0', 'hacknet-node-1', 'hacknet-node-2', 'hacknet-node-3', 'hacknet-node-4', 'hacknet-node-5', 'hacknet-node-6', 'hacknet-node-7', 'hacknet-node-8', 'hacknet-node-9', 'hacknet-node-10', 'hacknet-node-11', 'hacknet-node-12', 'hacknet-node-13', 'hacknet-node-14', 'hacknet-node-15', 'hacknet-node-16', 'hacknet-node-17', 'hacknet-node-18', 'hacknet-node-19'] //Servers names that won't be used as hosts or deleted
	...
	async function hnManager() {
		if (checkM(ns.hacknet.getPurchaseNodeCost(), divisor)) { ns.hacknet.purchaseNode() }
		for (let i = 0; i < ns.hacknet.numNodes(); i++) {
			for (let part of ['Level', 'Ram', 'Core']) { 
				if (checkM(ns.hacknet['get' + part + 'UpgradeCost'](i), divisor)) {
	...
	async function pServerManager() {
		let ram = 0; let ramList = [8]; for (let num of ramList) {
			if (num <= 1048576 && checkM(ns.getPurchasedServerCost(num), divisor)) { ramList.push(num * 2); ram = num; } else { break };
		}
		...
		for (let i = ns.getPurchasedServers().length - 1; i >= 0; i--) {
			...
			if (info('MR', tmp) < ram && checkM(ns.getPurchasedServerCost(ram), divisor) && !exclude.includes(tmp)) {
